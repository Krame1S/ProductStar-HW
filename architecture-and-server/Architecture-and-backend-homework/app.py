...
def create_app():
    ...
    def sglistsingle():
            ...
            if own == "Y":
                ...
                if lowest_comp_price != "NO COMP":
                    ...
                    # positive difference == lower bprice
                        ...
                        # SO THE TEAM WHEN THEY SEE BFLOOR WILL COMPARE TO THE 2ND
                        #  LOWEST AND -1 TO THEIR APROX
                        # BPRICE THEREFORE AVOIDING ANY WEIRD BPRICE FROM
                        # OTHER BROKERS WHO MAY TRY TO PUSH DOWN OUR PRICING.

                        return fv, "BFLOOR"
                    elif (difference > 0) & (size > ownsize):
                        return round(suggestedbprice, 2), "MRKDWN"
                    # negative difference = increase bprice but only if difference
                    # of more than 5 dollars (to monitor)
                    # need to find the 2nd lowest comp price that is "N" then push that.
                    # this would need to be seperated out as a seperate
                    # column with a different logic behind it.
        ...
        if ownsizedf.empty:
            ...
            # meanmindf = newdf.groupby('SECTION_NAME').agg(min_fv=('FACE_VALUE','min'),
            ...
            # to find the Mrkup 2nd lowest price only if there is
            # more than 1 count and if it is nochanges
            ...
            if "section" not in newdfcols:
                ...
                newdf.rename({"section": "SK CODE"}, inplace=True, axis=1)
                pass
            # newdf.drop(['row','section','SIZE','bprice','2ND_LOWEST'],
            # axis=1,inplace=True)
