...
def countsapi2(tid, sdate, edatestr):
    ...
    def notes(x, team):
        ...
        if bg == "Y":
            ...
            if (broad == "Y") & (prevbroad == "") & (qty == 1):
                ...
                if notes != "":
                    ...
                    bgnotes = (
                        "BG:%s -ticketevolution -tickpick "
                        "-ticketnetwork -stubhub -gametime -ticketmaster" % (newfv)
                    )
                    return bgnotes, "NA", "Rebroadcast"
        ...
        if fp == "Y":
            ...
            if (broad == "Y") | (qty == 1):
                bgnotes = (
                    "BG:%s -ticketevolution -tickpick -ticketnetwork -stubhub"
                    " -gametime -ticketmaster" % (newfv)
                )
                fpnotes = "FP:%s +vivid" % (newfv)
            ...
            for data in results:
                ...
                if broaddf.empty:
                    ...
                    cleandf[["bgnotes", "fpnotes", "newbroad"]] = cleandf.apply(
                        lambda x, t=teamname: notes(x, t), axis=1, result_type="expand"
                    )
